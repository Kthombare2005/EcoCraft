...
import PropTypes from "prop-types";
import { signInWithEmailAndPassword, signInWithPopup, signInWithRedirect, getRedirectResult, fetchSignInMethodsForEmail } from "firebase/auth";
import { auth, db, googleProvider, githubProvider } from "../../firebaseConfig";
import { doc, getDoc, setDoc } from "firebase/firestore";
import "./AuthPage.css";
import Image2 from "../../assets/Image2.png";
...
const Login = ({ onSwitch }) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      if (isMobileDevice()) {
        // Use redirect-based authentication for mobile devices
        await signInWithRedirect(auth, googleProvider);
      } else {
        // Use popup-based authentication for desktop devices
        const result = await signInWithPopup(auth, googleProvider);
  ...
  // Handle the result of Google login (for both popup and redirect)
  const handleGoogleLoginResult = async (result) => {
    ...
    if (signInMethods.length > 0) {
      // Email already exists, fetch user data from Firestore
      const userRef = doc(db, "users", user.uid);
      const docSnap = await getDoc(userRef);
      ...
      // Email doesn't exist, show account type selection modal
      setUser(user);
      setShowModal(true);
    }
  };
  ...
  useEffect(() => {
    const handleRedirectResult = async () => {
      try {
        ...
        setError("Failed to handle redirect result.");
